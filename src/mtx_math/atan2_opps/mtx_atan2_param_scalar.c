...
void	_mtx_iatan2f_pscalar(t_mtx *a, float b)
{
	...
	_mtx_opp_scalar(a, &mb, a, __mtx_std_atan2f);
}
...
void	_mtx_atan2f_pscalar(t_mtx *a, float b, t_mtx *out)
{
	...
	mb.arr = &b;
	mtx_shell(&mb, 1, 1, DTYPE_F);
	_mtx_opp_scalar(a, &mb, out, __mtx_std_atan2f);
}
...
void	_mtx_iatan2i_pscalar(t_mtx *a, int b)
{
	...
	_mtx_opp_scalar(a, &mb, a, __mtx_std_atan2i);
}
...
void	_mtx_atan2i_pscalar(t_mtx *a, int b, t_mtx *out)
{
	...
	mb.arr = &b;
	mtx_shell(&mb, 1, 1, DTYPE_I);
	_mtx_opp_scalar(a, &mb, out, __mtx_std_atan2i);
}
