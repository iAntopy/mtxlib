...
static t_mtx	*route_opp(t_mtx *a, t_mtx *b, t_mtx *out, int same_sh)
{
	static MOPP_FUNC	opps[2][6] = {
		{_mtx_subi_scalar, _mtx_rsubi_scalar, _mtx_subi_line_r,
		_mtx_rsubi_line_r, _mtx_subi_line_c, _mtx_rsubi_line_c},
		{_mtx_subf_scalar, _mtx_rsubf_scalar, _mtx_subf_line_r,
		_mtx_rsubf_line_r, _mtx_subf_line_c, _mtx_rsubf_line_c}};
	int					rev;
...
t_mtx	*mtx_sub(t_mtx *a, t_mtx *b, t_mtx *out)
{
	...
	if (same_shape && (a->dtype == DTYPE_F))
		_mtx_subf_mtx(a, b, ret);
	else if (same_shape && (a->dtype == DTYPE_I))
		_mtx_subi_mtx(a, b, ret);
	else
		route_opp(a, b, ret, same_shape);
	printf("mtx_sub SUCCESSFUL\n");
	return (ret);
}
