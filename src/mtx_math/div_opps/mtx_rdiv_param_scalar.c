...
void	_mtx_ridivf_pscalar(float a, t_mtx *b)
{
	...
	_mtx_opp_scalar(b, &ma, b, __mtx_std_rdivf);
}
...
void	_mtx_rdivf_pscalar(float a, t_mtx *b, t_mtx *out)
{
	...
	ma.arr = &a;
	mtx_shell(&ma, 1, 1, DTYPE_F);
	_mtx_opp_scalar(b, &ma, out, __mtx_std_rdivf);
}
...
void	_mtx_ridivi_pscalar(int a, t_mtx *b)
{
	...
	_mtx_opp_scalar(b, &ma, b, __mtx_std_rdivi);
}
...
void	_mtx_rdivi_pscalar(int a, t_mtx *b, t_mtx *out)
{
	...
	ma.arr = &a;
	mtx_shell(&ma, 1, 1, DTYPE_I);
	_mtx_opp_scalar(b, &ma, out, __mtx_std_rdivi);
}
