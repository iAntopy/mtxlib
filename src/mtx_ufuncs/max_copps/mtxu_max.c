...
/*   Updated: 2023/02/28 23:58:23 by iamongeo         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "mtxlib.h"

static t_mtx	*__route_max_rowwise(t_mtx *mtx, t_mtx *out)
{
	int	out_null;
	...
	if (mtx->dtype == DTYPE_F)
		_mtx_maxf_row(mtx, out);
	else if (mtx->dtype == DTYPE_I)
		_mtx_maxi_row(mtx, out);
	else if (out_null)
...
static t_mtx	*__route_max_colwise(t_mtx *mtx, t_mtx *out)
{
	int	out_null;
	...
	if (!(mtx->shape[0] == out->shape[0]))
		return (MTX_ERROR("invalid out shape"));
	if (mtx->dtype == DTYPE_F)
		_mtx_maxf_col(mtx, out);
	else if (mtx->dtype == DTYPE_I)
		_mtx_maxi_col(mtx, out);
	else if (out_null)
		return (MTX_ERR_CLR("invalid dtype", &out));
	else
		return (MTX_ERROR("invalid dtype"));
...
static t_mtx	*__route_max_whole(t_mtx *mtx, t_mtx *out)
{
	int	out_null;
	...
	if (!((out->shape[0] == 1) && out->shape[1] == 1))
		return (MTX_ERROR("invalid out shape"));
	if (mtx->dtype == DTYPE_F)
		_mtx_maxf_whole(mtx, out);
	else if (mtx->dtype == DTYPE_I)
		_mtx_maxi_whole(mtx, out);
	else if (out_null)
		return (MTX_ERR_CLR("invalid dtype", &out));
	else
		return (MTX_ERROR("invalid dtype"));
...
t_mtx	*mtx_max(t_mtx *mtx, int axis, t_mtx *out)
{
	...
	if (axis == ROWWISE)
		ret = __route_max_rowwise(mtx, ret);
	else if (axis == COLWISE)
		ret = __route_max_colwise(mtx, ret);
	else if (axis == WHOLE)
		ret = __route_max_whole(mtx, ret);
	else
